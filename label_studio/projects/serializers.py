...
class ProjectUserPermissionSerializer(serializers.ModelSerializer):
    """
    Serializes a User with a dynamic 'has_permission' field.

    Requires 'project' and 'permission_codename' to be passed in the serializer context.
    Example:
    context = {
        'project': project_object,
        'permission_codename': 'your_app.change_project'
    }
    """
    user_id = serializers.IntegerField(source='id', read_only=True)
    user_email = serializers.EmailField(source='email', read_only=True)

    # This field's value will be generated by the `get_has_permission` method.
    has_permission = serializers.SerializerMethodField()

    class Meta:
        model = User
        fields = ('user_id', 'user_email', 'has_permission')
